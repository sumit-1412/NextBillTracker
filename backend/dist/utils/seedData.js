...
const seedData = async () => {
    try {
        ...
        await Ward_1.default.deleteMany({});
        await Property_1.default.deleteMany({});
        console.log('Cleared existing data');
        // Create wards
        const wards = await Ward_1.default.insertMany([
            {
        ...
        for (const userInfo of userData) {
            const user = new User_1.default(userInfo);
            await user.save();
            users.push(user);
        }
        console.log('Created users:', users.length);
        // Create properties
        const properties = await Property_1.default.insertMany([
            {
                propertyId: 'PROP001',
                ward: wards[0]._id,
